...
def _prop_for_genetic_parition(jknife, M, Nbar, cat, tot, start_index, end_index):
    '''Convert total h2 and per-category h2 to per-category proportion h2 or gencov.'''
    #n_annot = self.n_annot
    python_end_index = end_index + 1
    n_annot = python_end_index - start_index
    n_blocks = jknife.delete_values.shape[0]
    numer_delete_vals = np.multiply(M, jknife.delete_values[:, start_index:python_end_index]) / Nbar  # (n_blocks, n_annot)
...
class LD_Score_Regression(object):
    ...
    def __init__(self, y, x, w, N, M, n_blocks, intercept=None, slow=False, step1_ii=None, old_weights=False, genetic_element_partitions=None):
        for i in [y, x, w, M, N]:
            try:
                if len(i.shape) != 2:
        ...
        if genetic_element_partitions is not None:
            self.partition_props = []
            self.partition_prop_covs = []
            genetic_parition_names, genetic_parition_indexes = load_in_genetic_parition_data(genetic_element_partitions)
            n_partitions = len(genetic_parition_names)
            for partition_index in range(n_partitions):
                ...
                partition_cat, partition_cat_cov, partition_cat_se = self._cat(jknife, M[:,start_pos:python_end_pos], Nbar, self.coef[start_pos:python_end_pos], self.coef_cov[start_pos:python_end_pos,start_pos:python_end_pos])
                partition_tot, partition_tot_cov, partition_tot_se = self._tot(partition_cat, partition_cat_cov)
                partition_prop, partition_prop_cov, partition_prop_se = _prop_for_genetic_parition(jknife, M[:,start_pos:python_end_pos], Nbar, partition_cat, partition_tot, start_pos, end_pos)
                
                self.partition_props.append(partition_prop)
                self.partition_prop_covs.append(partition_prop_cov)
...
class Hsq(LD_Score_Regression):
    ...
    def __init__(self, y, x, w, N, M, n_blocks=200, intercept=None, slow=False, twostep=None, old_weights=False, genetic_element_partitions=None):
        step1_ii = None
        if twostep is not None:
            step1_ii = y < twostep

        LD_Score_Regression.__init__(self, y, x, w, N, M, n_blocks, intercept=intercept,
                                     slow=slow, step1_ii=step1_ii, old_weights=old_weights, genetic_element_partitions=genetic_element_partitions)
