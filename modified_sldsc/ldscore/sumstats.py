...
def load_in_genetic_parition_data(genetic_partition_file):
    partition_names = []
    partition_bounds = []
    head_count = 0
    f = open(genetic_partition_file)
    for line in f:
        line = line.rstrip()
        data = line.split('\t')
        if head_count == 0:
            ...
            continue
        partition_names.append(data[0])
        partition_bounds.append(np.asarray([int(data[1]), int(data[2])]))
    f.close()
    return partition_names, partition_bounds

def extract_valid_genetic_elements(chrom_frq_file):
    dicti = {}
    f = open(chrom_frq_file)
    head_count = 0
    for line in f:
        line = line.rstrip()
        data = line.split()
        ...
        if maf < .05:
            continue
        dicti[element_name] = 1
    f.close()
    return dicti

def extract_partitioned_m_tot_on_a_chromosome(genetic_parition_indexes, valid_genetic_elements, chrom_annot_file):
    num_partitions = len(genetic_parition_indexes)
    chrom_m_tot = np.zeros(num_partitions)
    ...
    for line in f:
        ...
        if head_count == 0:
            head_count = head_count + 1
            continue
        if data[2] not in valid_genetic_elements:
            continue
        anno = data[4:]
        for part_num in range(num_partitions):
            if float(anno[genetic_parition_indexes[part_num][0]]) != 0.0:
                chrom_m_tot[part_num] = chrom_m_tot[part_num] + 1.0
    f.close()
    return chrom_m_tot

def extract_M_tot_genetic_partition(genetic_parition_indexes, ref_ld_chrom_stem, frq_file_chrom_stem):
    num_partitions = len(genetic_parition_indexes)
    M_tot_partitioned = np.zeros(num_partitions)
    for chrom_num in range(1,23):
        valid_genetic_elements = extract_valid_genetic_elements(frq_file_chrom_stem + str(chrom_num) + '.frq')
        chrom_m_tot_partitioned = extract_partitioned_m_tot_on_a_chromosome(genetic_parition_indexes, valid_genetic_elements, ref_ld_chrom_stem + str(chrom_num) + '.annot')
        M_tot_partitioned = M_tot_partitioned + chrom_m_tot_partitioned
    return M_tot_partitioned.astype(int)

def overlap_output_hack(category_names, overlap_matrix, M_annot, M_tot, print_coefficients, n_annot, prop, prop_cov, coef, coef_se, coef_cov, n_blocks):
    ...
    prop_hsq_overlap = np.dot(overlap_matrix_prop, prop.T).reshape((1, n_annot))
    prop_hsq_overlap_var = np.diag(np.dot(np.dot(overlap_matrix_prop, prop_cov), overlap_matrix_prop.T))
    prop_hsq_overlap_se = np.sqrt(np.maximum(0, prop_hsq_overlap_var)).reshape((1, n_annot))
    ...
    for i in range(n_annot):
        if not M_tot == M_annot[0,i]:
            overlap_matrix_diff[i, :] = overlap_matrix[i,:]/M_annot[0,i] - (M_annot - overlap_matrix[i,:]) / (M_tot-M_annot[0,i])
    diff_est = np.dot(overlap_matrix_diff,coef)
    diff_cov = np.dot(np.dot(overlap_matrix_diff,coef_cov),overlap_matrix_diff.T)
    diff_se = np.sqrt(np.diag(diff_cov))
    diff_p = ['NA' if diff_se[i]==0 else 2*tdist.sf(abs(diff_est[i]/diff_se[i]),n_blocks) for i in range(n_annot)]
    ...
    df = pd.DataFrame({
        ...
        'Enrichment_p':diff_p,
        'Coefficient': one_d_convert(coef),
        'Coefficient_std_error': coef_se,
        'Coefficient_z-score': one_d_convert(coef) / one_d_convert(coef_se)
    })
    if print_coefficients:
        df = df[['Category', 'Prop._SNPs', 'Prop._h2', 'Prop._h2_std_error','Enrichment','Enrichment_std_error', 'Enrichment_p','Coefficient', 'Coefficient_std_error','Coefficient_z-score']]
    else:
        df = df[['Category', 'Prop._SNPs', 'Prop._h2', 'Prop._h2_std_error','Enrichment','Enrichment_std_error', 'Enrichment_p']]
    return df

def _cat_hack(M, coef, coef_cov):
...
def estimate_h2(args, log):
    ...
    hsqhat = reg.Hsq(chisq, ref_ld, s(sumstats[w_ld_cname]), s(sumstats.N),
                     M_annot, n_blocks=n_blocks, intercept=args.intercept_h2,
                     twostep=args.two_step, old_weights=old_weights, genetic_element_partitions=args.genetic_element_partitions)
    ...
    if args.overlap_annot:
        if args.genetic_element_partitions is None:
            ...
            genetic_parition_names, genetic_parition_indexes = load_in_genetic_parition_data(args.genetic_element_partitions)
            M_tot_partition = extract_M_tot_genetic_partition(genetic_parition_indexes, args.ref_ld_chr, args.frqfile_chr)
            overlap_matrix, M_tot = _read_annot(args, log)
            partition_dfs = []
            for partition_index, partition_name in enumerate(genetic_parition_names):
                start_pos = genetic_parition_indexes[partition_index][0]
                end_pos = genetic_parition_indexes[partition_index][1]
                python_end_pos = end_pos + 1
                num_anno = end_pos-start_pos+1
                partition_df_results = overlap_output_hack(ref_ld_cnames[start_pos:python_end_pos], overlap_matrix[start_pos:python_end_pos,start_pos:python_end_pos], M_annot[:,start_pos:python_end_pos], M_tot_partition[partition_index], args.print_coefficients, num_anno, hsqhat.partition_props[partition_index], hsqhat.partition_prop_covs[partition_index], hsqhat.coef[start_pos:python_end_pos], hsqhat.coef_se[start_pos:python_end_pos], hsqhat.coef_cov[start_pos:python_end_pos,start_pos:python_end_pos], hsqhat.n_blocks)
                partition_dfs.append(partition_df_results)
            cat_df_results = pd.concat(partition_dfs)
            cat_df_results.to_csv(args.out+'.results', sep="\t", index=False)
            log.log('Results printed to '+args.out+'.results')
    return hsqhat
